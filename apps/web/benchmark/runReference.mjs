...
import { assertSanitized, assertStaticBudgets, loadBudgets, nearestRank, sha256, summarizeFrameWindow, summarizeStaticAssets } from "./releaseMetrics.mjs";

const appRoot = resolve(dirname(fileURLToPath(import.meta.url)), ".."); const repositoryRoot = resolve(appRoot, "../.."); const distRoot = join(appRoot, "dist");
...
const wait = (milliseconds) => new Promise((done) => setTimeout(done, milliseconds)); const parse = async (path) => JSON.parse(await readFile(path, "utf8"));

function runNode(arguments_, environment = process.env) {
  const run = spawnSync(process.execPath, arguments_, { cwd: appRoot, env: environment, stdio: "inherit" }); if (run.status !== 0) throw new Error("benchmark.command.failed");
}

async function prepare(budgets, previewRoot) {
  const manifestBytes = await readFile(join(bundleRoot, "manifest.json")); const manifest = JSON.parse(manifestBytes);
  let candidateBytes = manifestBytes.length; const bundledAssets = [];
  for (const asset of manifest.assets) { const bytes = await readFile(join(bundleRoot, asset.locator.path));
    if (bytes.length !== asset.size_bytes || sha256(bytes) !== asset.sha256) throw new Error("benchmark.bundle.invalid");
    candidateBytes += bytes.length; bundledAssets.push([asset, bytes]); }
  ...
  const staticAssets = await summarizeStaticAssets(distRoot); assertStaticBudgets(staticAssets, budgets.staticAssets);
  await cp(distRoot, previewRoot, { recursive: true });
  const candidateRoot = join(previewRoot, "models/candidate"); await mkdir(candidateRoot, { recursive: true });
  await writeFile(join(candidateRoot, "manifest.json"), manifestBytes);
  for (const [asset, bytes] of bundledAssets) { const target = join(candidateRoot, asset.locator.path);
    await mkdir(dirname(target), { recursive: true }); await writeFile(target, bytes); }
  const taskLock = await parse(taskLockPath); const taskRoot = join(previewRoot, "models/mediapipe"); await mkdir(taskRoot, { recursive: true });
  for (const task of taskLock.tasks) {
    const response = await fetch(task.source_url); const bytes = Buffer.from(await response.arrayBuffer());
...
async function preview(root) {
  const port = 4176; const server = spawn(process.execPath, [join(appRoot, "node_modules/vite/bin/vite.js"), "preview", "--outDir", root, "--host", "127.0.0.1", "--port", `${port}`, "--strictPort"], { cwd: appRoot, stdio: "ignore" });
  for (let attempt = 0; attempt < 50; attempt += 1) {
    try { if ((await fetch(`http://127.0.0.1:${port}`)).ok) return { origin: `http://127.0.0.1:${port}`, server }; } catch {}
    if (server.exitCode !== null) break;
    await wait(100);
  }
  server.kill(); throw new Error("benchmark.preview.failed");
}
...
async function measurePage(context, origin, workerUrl, input) {
  ...
  await page.goto(`${origin}/#/live`); await page.getByRole("button", { name: "Start camera" }).click();
  ...
  await page.getByRole("button", { name: "Stop camera" }).click(); const inference = await measureInference(page, workerUrl, input);
  const longTasks = await page.evaluate(() => globalThis.__signlabLongTasks);
  const processedFrames = end["Processed frames"] - start["Processed frames"]; const droppedFrames = end["Dropped frames"] - start["Dropped frames"];
  const summary = summarizeFrameWindow(processedFrames, droppedFrames, elapsedSeconds, longTasks);
  await page.close();
  return { startupMs, landmarks: { fixture: "chromium_fake_camera_640x480_20fps_no_person", windowSeconds: elapsedSeconds,
      processedFrames, droppedFrames, processedFps: summary.processedFps, dropRate: summary.dropRate }, inference,
    uiLongTasks: summary.uiLongTasks,
...
function markdown(report) {
  ...
  return `# Browser reference baseline v1\n\n> Reference-machine evidence only; timings are not cross-device guarantees or CI thresholds.\n\n- Browser: ${report.environment.browser}\n- Machine: ${report.environment.os}, ${report.environment.cpu}, ${report.environment.logicalCpuCount} logical CPUs, ${report.environment.memoryGiB} GiB RAM\n- Toolchain: Node ${report.environment.node}, npm ${report.environment.npm}\n- Runtime: WASM, one thread\n- Fixture: Chromium's generated no-person 640×480 camera; 50 public synthetic candidate events after 5 warmups\n- Percentiles: nearest-rank over preprocessing + ONNX + decision time\n\n| Run | Startup ms | Landmark FPS | Dropped | Inference p50 ms | Inference p95 ms | UI long tasks |\n| --- | ---: | ---: | ---: | ---: | ---: | ---: |\n${row("Cold cache", report.measurements.cold)}\n${row("Warm cache", report.measurements.warm)}\n\nMemory: unavailable because this release does not enable cross-origin isolation.\n\nStatic assets: ${report.assets.static.totalBytes} bytes; candidate bundle: ${report.assets.candidate.totalBytes} bytes; MediaPipe tasks: ${report.assets.mediapipe.totalBytes} bytes.\n\n## Limits\n\n${report.limitations.map((item) => `- ${item}`).join("\n")}\n`;
}

const budgets = await loadBudgets(); const packageConfig = await parse(join(appRoot, "package.json"));
const npmAgent = process.env.npm_config_user_agent?.split(" ")[0]; if (process.version !== `v${packageConfig.engines.node}` || npmAgent !== packageConfig.packageManager.replace("@", "/")) throw new Error("benchmark.toolchain.invalid");
const dirty = spawnSync("git", ["status", "--porcelain=v1", "--untracked-files=all", "--", ".", ":(exclude)docs/reports/browser-reference-v1.json", ":(exclude)docs/reports/browser-reference-v1.md"], { cwd: repositoryRoot, encoding: "utf8" }); if (dirty.status !== 0 || dirty.stdout.trim()) throw new Error("benchmark.source.dirty");
const sourceCommit = spawnSync("git", ["rev-parse", "HEAD"], { cwd: repositoryRoot, encoding: "utf8" }).stdout.trim(); if (!existsSync(chromium.executablePath())) runNode([join(appRoot, "node_modules/playwright/cli.js"), "install", "chromium"]);
const previewRoot = await mkdtemp(join(tmpdir(), "signlab-browser-reference-")); let server; let browser; let context;
try {
const assets = await prepare(budgets, previewRoot); const started = await preview(previewRoot); server = started.server;
const workerName = (await readdir(join(previewRoot, "assets"))).find((name) => /^candidateInference\.worker-.*\.js$/u.test(name)); if (!workerName) throw new Error("benchmark.worker.missing");
const fixtureBytes = await readFile(fixturePath); const fixture = JSON.parse(fixtureBytes);
const source = fixture.preprocessingCases.find(({ id }) => id === "gap_unmirrored"); const input = { frames: source.frames, sourceMirrorState: source.sourceMirrorState, quality: source.quality };
browser = await chromium.launch({ headless: true, args: ["--use-fake-device-for-media-stream", "--use-fake-ui-for-media-stream"] }); context = await browser.newContext({ permissions: ["camera"] });
await context.addInitScript(() => {
  ...
  const cold = await measurePage(context, started.origin, `${started.origin}/assets/${workerName}`, input);
  const warm = await measurePage(context, started.origin, `${started.origin}/assets/${workerName}`, input);
  ...
  const report = { format: "signlab-browser-reference/1", recordedAt: new Date().toISOString(), sourceCommit,
    environment: { browser: `Chromium ${browser.version()}`, node: process.version.slice(1), npm: npmAgent.slice(4), os: `${platform()} ${release()} ${process.arch}`,
  ...
  const reportRoot = join(repositoryRoot, "docs/reports"); await writeFile(join(reportRoot, "browser-reference-v1.json"), `${JSON.stringify(report)}\n`);

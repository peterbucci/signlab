"""Regenerate or check extraction schemas, config, model lock, and Arrow snapshot."""
...
from signlab.extraction.resources import generated_extraction_resource_texts

REPOSITORY_ROOT = Path(__file__).resolve().parents[1]
RESOURCE_DIRECTORY = REPOSITORY_ROOT / "src" / "signlab" / "resources" / "extraction"


def write_resources(directory: Path = RESOURCE_DIRECTORY) -> tuple[Path, ...]:
    """Write every generated extraction resource in stable path order."""

    written: list[Path] = []
    for relative_name, content in sorted(generated_extraction_resource_texts().items()):

"""Regenerate or check portable feature schemas and default plans."""
...
from signlab.features.resources import generated_feature_resource_texts

REPOSITORY_ROOT = Path(__file__).resolve().parents[1]
RESOURCE_DIRECTORY = REPOSITORY_ROOT / "src" / "signlab" / "resources" / "features"


def write_resources(directory: Path = RESOURCE_DIRECTORY) -> tuple[Path, ...]:
    """Write every generated feature resource in stable path order."""

    written: list[Path] = []
    for relative_name, content in sorted(generated_feature_resource_texts().items()):
